...
void getCoinCombinations(long coins[], int n, int amount) {
    ...
    // 4D array to store the combinations
    vector<vector<vector<vector<int>>>> ways(amount + 1, vector<vector<vector<int>>>(n + 1));

    for (int i = 0; i <= n; ++i) {
        // initialize the first column to be empty
        ways[0][i].push_back({});
    }

    // loop through the amount
    for (int i = 1; i <= amount; ++i) {
        // loop through the number of amounts
        for (int j = 1; j <= n; ++j) {
            // if the current amount is greater than the current coin
            if (i - coins[j - 1] >= 0) {
                ...
                for (int k = 0; k < ways[i - coins[j - 1]][j].size(); ++k) {
                    ...
                    // add the combination to the ways array
            ...
            // add the combinations from the previous column
            if (j > 1) {
